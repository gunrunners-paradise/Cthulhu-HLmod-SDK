...
BitmapTGA *LoadTGAForRes( const char* pImageName )
{
	...
	sprintf(tgasz,"%s",GetTGANameForRes(sz));

	//pTGA = vgui_LoadTGA(GetTGANameForRes(sz));
	pTGA = vgui_LoadTGA(tgasz);

	// HACK HERE
	// For some reason, I am not seeing the TFC folder.
	// So I will force it...
	if (pTGA == NULL)
	{
		char tgasz2[256];
		sprintf(tgasz2,"..\\tfc\\%s",tgasz);
		pTGA = vgui_LoadTGA(tgasz2);
	}
