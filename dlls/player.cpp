...
void CBasePlayer::GoInsane ( void )
{
	...
	strcpy(st_szNextSpot, "lm_insane");
	strcpy(st_szNextMap, "insane");
...
void CBasePlayer::RemoveItems( int iWeaponMask, int i9mm, int i357, int iBuck, int iBolt, int iARGren, int iRock, int iUranium, int iSatchel, int iSnark, int iTrip, int iGren, int iHornet )
{
	...
	// hornetgun is outside the spawnflags Worldcraft can set - handle it
	// seperately.
//	if (iHornet)
...
{
	//LRC - used to cap DesiredList. Assume there will be at most 100 items in the list.
	int loopbreaker = 100; 
...
// Copied from spawn() for now
	...
	if (FStrEq(STRING(gpGlobals->mapname),"insane"))
	{
		// find the landmark
		edict_t* lmpev = CChangeLevel::FindLandmark("lm_insane");
		if (lmpev)
		{
...
void CBasePlayer :: UpdateClientData( void )
{
	...
	if (pev->sanity != m_iClientSanity)
	{
		m_iClientSanity = pev->sanity;
