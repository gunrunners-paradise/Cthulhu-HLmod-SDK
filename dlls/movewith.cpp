...
void AddToAssistList( CBaseEntity *pEnt )
{
	if (pEnt->m_pAssistLink)
	{
		//ALERT(at_console, "Ignored AddToAssistList for %s\n", STRING(pEnt->pev->targetname));
	...
	if ( g_pWorld )
	{
		...
		if (pListMember == pEnt)
		{
			//ALERT(at_console, "(end)Ignored AddToAssistList for %s\n", STRING(pEnt->pev->targetname));
	...
	{
		ALERT(at_debug, "AddToAssistList has no world!\n");
	}

//	ALERT(at_console, "AddToAssistList %p ok\n", pEnt->pev);
}
...
void ApplyDesiredSettings( CBaseEntity *pListMember )
{
...
//
// this function returns TRUE if the entity no longer needs assistance.
void TryAssistEntity( CBaseEntity *pEnt )
{
...
//		ALERT(at_console, "Cancelling assist for %s, %f\n", STRING(pEnt->pev->targetname), pEnt->pev->origin.x);
		...
		return; // the think has been cancelled. Oh well...
	}
...
//		ALERT(at_console, "Setting DesiredThink %f\n", pEnt->pev->origin.x);
...
void CheckAssistList( void )
{
	...
	if (!g_pWorld)
	{
		ALERT(at_debug, "AssistList has no world %f\n", gpGlobals->time);
		return;
	}
	...
	while ( pListMember->m_pAssistLink ) // handle the remaining entries in the list
	{
		...
		{
			// should we remove this entity from the list now?

			//ALERT(at_console, "Assist complete, removing %s\n", STRING(pListMember->pev->targetname));
			CBaseEntity *pTemp = pListMember->m_pAssistLink->m_pAssistLink;
			pListMember->m_pAssistLink->m_pAssistLink = NULL;
			pListMember->m_pAssistLink = pTemp;
		}
		else
		{
			pListMember = pListMember->m_pAssistLink;
...
//	}
	...
	if ( pEnt->m_pChildMoveWith )
	{
		...
		int sloopbreaker = 100; // to prevent the game hanging...
		while (pMoving)
		{
...
//	);
	...
	{
		CBaseEntity *pMoving = pEnt->m_pChildMoveWith;
		int sloopbreaker = 100; // LRC - to save us from infinite loops
		while (pMoving)
		{
			UTIL_SetMoveWithVelocity(pMoving, vecNew, 100 );
			pMoving = pMoving->m_pSiblingMoveWith;
			sloopbreaker--;
			if (sloopbreaker <= 0)
			{
...
void UTIL_SetAvelocity ( CBaseEntity *pEnt, const Vector vecSet )
{
	...
	if ( pEnt->m_pChildMoveWith )
	{
		...
		while (pMoving)
		{
			UTIL_SetMoveWithAvelocity(pMoving, vecNew, 100 );
			pMoving = pMoving->m_pSiblingMoveWith;
			sloopbreaker--;
			if (sloopbreaker <= 0)
			{
