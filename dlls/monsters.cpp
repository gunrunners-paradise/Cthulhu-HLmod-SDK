...
BOOL CBaseMonster :: CheckMeleeAttack1 ( float flDot, float flDist )
{
	...
	if ( flDist <= 64 && flDot >= 0.7 && m_hEnemy != NULL && bHit )
	{
		return TRUE;
	}
	return FALSE;
...
// last ditch, try nodes
	...
	{
//		ALERT ( at_debug, "Can get there on nodes\n" );
...
BOOL CBaseMonster :: RunAwayFromEnemy ( void )
{
	...
	if ( MoveToLocation( ACT_RUN, 0, pev->origin+(run_away*tr.flFraction*0.95) ) )
	{
		return TRUE;
	}
