...
//		ALERT(at_debug, "Unset Retrigger (use)\n");
		pev->spawnflags &= ~SF_TRAIN_WAIT_RETRIGGER;
		Next();
	}
	else
	{
//		ALERT(at_debug, "Set Retrigger (use)\n");
		pev->spawnflags |= SF_TRAIN_WAIT_RETRIGGER;
...
//			ALERT(at_debug, "Wait: wait for retrigger from path %s\n", STRING(m_pevCurrentTarget->targetname));
//		else
//			ALERT(at_debug, "Wait: wait for retrigger from train\n");
		pev->spawnflags |= SF_TRAIN_WAIT_RETRIGGER;
	...
	{// -1 wait will wait forever!		
		...
		if ( pev->noiseStopMoving )
			EMIT_SOUND (ENT(pev), CHAN_VOICE, (char*)STRING(pev->noiseStopMoving), m_volume, ATTN_NORM);
		SetThink( Next );
//		ALERT(at_debug, "Wait: doing Next in %f\n", m_flWait);
	}
	else
	{
//		ALERT(at_debug, "Wait: doing Next now\n");
...
//	ALERT(at_debug, "Next, new pevtarget %s, new message %s\n", STRING(pev->target), STRING(pev->message));
	...
	if ( m_pevCurrentTarget )
	{
		...
		{
			UTIL_SetAvelocity(this, pTarg->pev->avelocity);
...
//		if (pTarg->pev->armorvalue) //LRC - "turn to face" the next corner
		{
			...
			UTIL_SetAvelocity( this, aDelta / timeTaken );
...
//		ALERT(at_debug, "Next: LMove done\n");
		//ALERT(at_debug, "Next ends, nextthink %f, flags %f\n", pev->nextthink, m_iLFlags);
	}
}
...
//		SetThink( Next );
	...
	{
//		ALERT(at_debug, "Set Retrigger (postspawn)\n");
		pev->spawnflags |= SF_TRAIN_WAIT_RETRIGGER;
	}

//	ALERT(at_debug, "func_train postspawn: origin %f %f %f\n", pev->origin.x, pev->origin.y, pev->origin.z);
}
...
//	ALERT(at_debug, "Unset Retrigger (startsequence)\n");
	pev->spawnflags &= ~SF_TRAIN_WAIT_RETRIGGER;
	//...
}
...
void CFuncTrain :: StopSequence( )
{
	m_pSequence = NULL;
	pev->spawnflags &= ~SF_TRAIN_WAIT_RETRIGGER;
	if (pev->spawnflags & SF_TRAIN_REVERSE)
	{
		pev->spawnflags &= ~SF_TRAIN_REVERSE;
//		ALERT(at_debug, "StopSequence t %s, m %s\n", STRING(pev->target), STRING(pev->message));
	}
...
//	ALERT(at_debug, "SoundSetup: mfNT %f, pevNT %f, stored was %f, time %f", m_fNextThink, pev->nextthink, m_fStoredThink, pev->ltime );
	m_fStoredThink = 0;
...
void CFuncTrain :: Precache( void )
{
	...
	if (m_iState == STATE_ON && pev->noiseMovement)
	{
		...
		SetNextThink( 0.1 );
//		ALERT(at_debug, "preparing SoundSetup: stored %f, mfNT %f, pevNT %f, ltime %f", m_fStoredThink, m_fNextThink, pev->nextthink, pev->ltime);
		SetThink( SoundSetup );
	}
...
void CFuncTrackTrain :: Spawn( void )
{
	...
	UTIL_SetSize( pev, pev->mins, pev->maxs );
	UTIL_SetOrigin( this, pev->origin );
//	ALERT(at_debug, "SpawnOrigin %f %f %f\n", pev->origin.x, pev->origin.y, pev->origin.z);
...
//	ALERT(at_debug, "set trainvel %f %f %f\n", m_train->pev->velocity.x, m_train->pev->velocity.y, m_train->pev->velocity.z);
}
...
//			ALERT(at_debug, "ArrivalNotify %s stop\n", STRING(pev->targetname));
			EndThink();
		}
		else
		{
//			ALERT(at_debug, "ArrivalNotify %s continue\n", STRING(pev->targetname));
		}
	}
...
void CTrainSequence :: StopSequence()
{
	if (m_pTrain)
	{
//		ALERT(at_debug, "StopSequence called\n");
		//stuff...
