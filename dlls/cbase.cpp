...
//				ALERT( at_debug, "Added global entity %s (%s)\n", STRING(pEntity->pev->classname), STRING(pEntity->pev->globalname) );
			}
		}
...
void DispatchThink( edict_t *pent )
{
	...
	if (pEntity)
	{
		...
		//if (pEntity->pev->classname) ALERT(at_debug, "DispatchThink %s\n", STRING(pEntity->pev->targetname));
		pEntity->Think();
	}
}
...
//		ALERT(at_debug,"Init: %s moves with %s\n", STRING(pev->classname), STRING(m_MoveWith));
	...
	if (!pSibling) // if movewith is being set up for the first time...
	{
		...
		{
			if (pev->solid == SOLID_BSP) // not sure whether that's the right test to make...
		...
		{
			//ALERT(at_debug,"Corrected using SpawnOffset\n");
...
//	ALERT(at_debug, "DontThink for %s\n", STRING(pev->targetname));
}
...
//		ALERT(at_debug, "SetAssistedThink for %s: %f\n", STRING(pev->targetname), m_fNextThink);
	}
	else
	{
...
//		if (pev->classname) ALERT(at_debug, "SetNormThink for %s: %f\n", STRING(pev->targetname), m_fNextThink);
	}
}
...
TYPEDESCRIPTION	CBaseEntity::m_SaveData[] = 
{
	...
	DEFINE_FIELD( CBaseEntity, m_pAssistLink, FIELD_CLASSPTR ), //LRC
