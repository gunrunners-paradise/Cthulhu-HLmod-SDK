...
void CBaseMonster :: MaintainSchedule ( void )
{
	...
	// validate existing schedule 
		if ( !FScheduleValid() || m_MonsterState != m_IdealMonsterState )
		{
			...
			{
				BOOL b1 = (m_afConditions && !HasConditions(bits_COND_SCHEDULE_DONE));
				BOOL b2 = (m_pSchedule && (m_pSchedule->iInterruptMask & bits_COND_SCHEDULE_DONE));
				BOOL b3 = ((m_MonsterState == MONSTERSTATE_COMBAT) && (m_hEnemy == NULL));
				if ( b1 || b2 || b3)
				{
					GetIdealState();
				}
...
//				return;
			...
			else if (RunAwayFromEnemy())
			{
				// just simple run away...
				m_flMoveWaitFinished = gpGlobals->time + pTask->flData;
				TaskComplete();
			}
			else
	...
	case TASK_WALK_TO_SCRIPT:
		{
			...
			if ( !m_pCine || (m_pCine->pev->origin - pev->origin).Length() < 1 )
				TaskComplete();
			else
			{
				...
				{
					if ( m_pCine == NULL || !MoveToLocation( newActivity, 2, m_pCine->pev->origin ) )
					{
						TaskFail();
...
case TASK_GET_PATH_TO_BESTSCENT:
	...
	case TASK_WAIT_FOR_SCRIPT:
		{
			if (!m_pCine->IsAction() && m_pCine->m_iszIdle)
			{
	...
	case TASK_PLANT_ON_SCRIPT:
		{
			if ( m_pCine != NULL )
			{
				pev->origin = m_pCine->pev->origin;	// Plant on script
			}
