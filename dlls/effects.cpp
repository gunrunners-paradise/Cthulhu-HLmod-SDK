...
int IsPointEntity( CBaseEntity *pEnt )
{
	...
	if (pEnt->pev->modelindex) //LRC- follow any entity that has a model
		return 0;
	else
		return 1;
}
...
void CLaser::Spawn( void )
{
	...
	else if ( pev->spawnflags & SF_LASER_INTERPOLATE)
		m_pSprite = CSprite::SpriteCreate( "sprites/null.spr", pev->origin, TRUE ); //LRC
	else
		m_pSprite = NULL;

	//LRC
	if ( m_pSprite && pev->spawnflags & SF_LASER_INTERPOLATE )
		PointEntInit( pev->origin, m_pSprite->entindex() );
	else
...
void CLaser::TurnOff( void )
{
	...
	{
		m_pSprite->TurnOff();
		m_pSprite->pev->velocity = g_vecZero; //LRC
	}
}
...
void CLaser::FireAtPoint( TraceResult &tr )
{
	...
	if (m_pSprite)
	{
		if (pev->spawnflags & SF_LASER_INTERPOLATE)
		{
			m_pSprite->pev->velocity = (tr.vecEndPos - m_pSprite->pev->origin)*10; //LRC
		}
		else
		{
...
//	UTIL_TraceLine( pev->origin, m_firePosition, dont_ignore_monsters, NULL, &tr );
	...
	if ( m_iProjection )
	{
		Vector vecProject = pev->origin + 4096*((m_firePosition - pev->origin).Normalize());
		UTIL_TraceLine( pev->origin, vecProject, iIgnoreMonsters, iIgnoreGlass, NULL, &tr );
	}
	else
	{
		UTIL_TraceLine( pev->origin, m_firePosition, iIgnoreMonsters, iIgnoreGlass, NULL, &tr );
	}
...
void CGibShooter :: ShootThink ( void )
{
	...
	while (i > 0)
	{
		Vector vecShootDir;
		Vector vecPos;
		float flGibVelocity = CalcLocus_Ratio(m_hActivator, STRING(m_iszVelFactor));
...
CBaseEntity *CEnvShooter :: CreateGib ( Vector vecPos, Vector vecVel )
{
	...
	switch (m_iPhysics)
	{
	case 2: pShot->pev->movetype = MOVETYPE_NOCLIP; break;
...
//==================================================================
//LRC- env_dlight; Dynamic Light creator
//==================================================================
...
void CEnvDLight::Use( CBaseEntity *pActivator, CBaseEntity *pCaller, USE_TYPE useType, float value )
{
	...
	MESSAGE_BEGIN( MSG_PVS, SVC_TEMPENTITY, pev->origin );
		WRITE_BYTE(TE_DLIGHT);
		WRITE_COORD( vecPos.x );		// X
		WRITE_COORD( vecPos.y );		// Y
		WRITE_COORD( vecPos.z );		// Z
		...
		WRITE_BYTE( pev->health * 10 );		// time * 10
...
{
	CBaseEntity *pAttach = this;
	Vector vecPos = pev->origin;
	if (pev->target)
	{
		pAttach = UTIL_FindEntityByTargetname( NULL, STRING(pev->target), pActivator);
		if (!pAttach) return; // error?
	}
	...
	MESSAGE_BEGIN( MSG_PVS, SVC_TEMPENTITY, pev->origin );
		...
		WRITE_SHORT( pAttach->entindex( ) + 0x1000 * pev->impulse );		// entity, attachment
		WRITE_COORD( vecPos.x );		// X
		WRITE_COORD( vecPos.y );		// Y
		WRITE_COORD( vecPos.z );		// Z
...
void CEnvDecal::Use( CBaseEntity *pActivator, CBaseEntity *pCaller, USE_TYPE useType, float value )
{
	...
	Vector vecPos = CalcLocus_Position( this, pActivator, STRING(pev->target) );
	Vector vecOffs = CalcLocus_Velocity( this, pActivator, STRING(pev->netname) );
	if (pev->message)
